...
export default function ({ cards, onClick }: Props) {
  ...
  return (
      <svg viewBox={`0 0 ${width} ${svgCardHeight}`} height="3.5in">
        {cards.map((card, i) => (
          <SvgCard style={{cursor: "grab"}} onClick={_ => onClick!== undefined && onClick(card, i)} card={card} key={`card${i}`} width={svgCardWidth} height={svgCardHeight} transform={`translate(${translate * i},0)`} />
        ))}
      </svg>
  );
}
