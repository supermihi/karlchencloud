...
func NewBotClient(login client.LoginData, isOwner bool, invite string, delay time.Duration) *BotClient {
  bot := &BotClient{isOwner: isOwner, invite: invite, delay: delay}
...
func (h *BotClient) OnMemberJoin(_ string, _ string) {
  if len(h.Table().MemberNamesById) >= 4 && h.isOwner {
    time.Sleep(h.delay)
    h.Logf("I'm the owner, starting table ...")
    err := h.StartTable()
    if err != nil {
